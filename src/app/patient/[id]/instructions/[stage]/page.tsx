  ...
  if (!stage) {
    return (
      <div className="animate-fade-in p-6 text-center">
        ...
        <Link href={`/patient/${params.id}`} className="text-blue-600 text-sm mt-2 inline-block hover:underline">
          대시보드로 돌아가기
        </Link>
      </div>
    );
  ...
  return (
    <div className="animate-fade-in space-y-4 pb-24">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        ...
        <Link href={`/patient/${params.id}`} className="text-[10px] text-blue-500 hover:underline mt-1 inline-block">
          대시보드로 이동
        </Link>
      </div>
      ...
      <div className="flex justify-between gap-2">
        {prevStage ? (
          <Link
            href={`/patient/${params.id}/instructions/${prevStage.id}`}
        ...
        {nextStage ? (
          <Link
            href={`/patient/${params.id}/instructions/${nextStage.id}`}
            className="flex-1 bg-white border border-gray-200 rounded-xl p-2.5 text-center text-sm hover:bg-gray-50 hover:border-blue-200 transition-colors"
          >
